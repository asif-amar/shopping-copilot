...
export const ProductCard: React.FC<ProductCardProps> = ({ product, index = 0 }) => {
  ...
  const isAvailable = product.availability?.includes("זמין") || product.availability?.toLowerCase().includes("in stock");
  
  // Parse rating if available
  const rating = product.rating ? parseFloat(product.rating.split("/")[0]) : null;
  ...
  return (
    <motion.div
      ...
      style={{
        ...
        cursor: "pointer",
        boxShadow: isHovered 
          ? "0 20px 40px -8px rgba(0, 0, 0, 0.12), 0 0 0 1px rgba(102, 126, 234, 0.15)"
