...
import { ShufersalHeaders } from "@shopping-copilot/shared";
...
export class ShufersalAdapter extends BaseShoppingAdapter {
  ...
  private createAuthHeaders(credentials: ShufersalHeaders): Record<string, string> {
    return {
  ...
  async addToCart(
    ...
    try {
      ...
      if (!csrfToken || !cookie) {
        return this.createErrorResult(
          "Missing Shufersal authentication credentials. Please provide CSRF_TOKEN and COOKIE via request headers."
        );
      }
  ...
  async updateCartQuantity(_cartItemId: string, _quantity: number, _credentials: ShufersalHeaders): Promise<ShoppingOperationResult<CartItem>> {
    return this.createErrorResult(
      "Cart operations not implemented for Shufersal"
    );
  }

  async getCartContents(credentials: ShufersalHeaders): Promise<ShoppingOperationResult<Cart>> {
    try {
